...
RSpec.describe User, type: :model do
  ...
  it { is_expected.to allow_value('lucas@gomes.com').for(:email) }
  it { is_expected.to validate_uniqueness_of(:auth_token) }

  describe '#info' do
    it 'returns email and created_t and a token' do
      user.save!
  ...
  describe '#generate_authentication_token!' do
    ...
    it 'generates another auth token when the current auth token already has been taken' do
      allow(Devise).to receive(:friendly_token).and_return('token', 'token', 'new_token')
